...
export const API_URLS = {
  GET_ALL_REQUEST_STUDENT: `${URL}/user/admin/${endPoints.pendingDeleteRequests}`,
  DELETE_REQUEST_STUDENT: `${URL}/user/admin/${endPoints.deleteRequestStatus}`,
};

export const getAllRequestStudent = async (postData) => {
  try {
    const url = `${API_URLS.GET_ALL_REQUEST_STUDENT}`;
...
export const changeRequestStudent = async (postData) => {
  try {
    console.log("postData", postData); // Debugging response
    const url = API_URLS.DELETE_REQUEST_STUDENT;
    console.log("API Call URL:", url); // Debugging URL
    const payload = {
      userId: postData.UserId, // Match the expected format
