...
export const API_URLS = {
  GET_ALL_REQUEST_PARENT: `${URL}/parent/admin/${endPoints.pendingDeleteRequests}`,
  DELETE_REQUEST_PARENT: `${URL}/parent/admin/${endPoints.deleteRequestStatus}`,
};

export const getAllParentRequest = async (postData) => {
  try {
    const url = `${API_URLS.GET_ALL_REQUEST_PARENT}`;
...
export const changeParentRequest = async (postData) => {
  try {
    console.log("postData", postData); // Debugging response
    const url = `${API_URLS.DELETE_REQUEST_PARENT}`;
    console.log("API Call URL:", url); // Debugging URL
    const payload = {
      parentId: postData.parentId, // Match the expected format
